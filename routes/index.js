...
router.post('/change-password', isLoggedIn, async (req, res) => {
  ...
  if (!user) {
    return res.render('change-password', {error: 'User not found', nav: false });
  }

  const { oldPassword, newPassword, confirmPassword } = req.body;

  if (newPassword !== confirmPassword) {
    return res.render('change-password', { error: 'New passwords do not match', nav: false });
  }
  ...
  user.changePassword(oldPassword, newPassword)
    ...
    .catch((error) => {
      console.error(error);
      res.render('change-password', {error: 'User not found', nav: false });
    });
});
...
router.post('/changepostdetail/:CardId', isLoggedIn, async function(req, res, next) {
  const user = await userModel.findOne({username: req.session.passport.user}).populate("posts")
  const post = await postModel.findOneAndUpdate({ _id: req.params.CardId },{title: req.body.title, description: req.body.description}, {new: true})
  res.redirect('back');
});
...
router.post('/changedetail',isLoggedIn, async function(req, res) {
  const user = await userModel.findOneAndUpdate({username: req.session.passport.user}, {username: req.body.username, name: req.body.name}, {new: true})
